...
public class CalcTwentyFour {
	...
	private static String solve(List<Integer> list) {
		...
		List<FractionExpression> f0 = new ArrayList<FractionExpression>();
		for (Integer n : list) {
			f0.add(new Fraction(n));
		}
		mp.put(getKey(f0), f0);
			...
			for (List<FractionExpression> currList : mp.values()) {
				...
				for (int i = 0; i < s-1; i++) {
					for (int j = i + 1; j < s; j++) {
						...
						List<FractionExpression> remains = getRemainItems(currList, s, i, j);
						List<FractionExpression> combo = getAllCombo(currList, i, j);
